...
fn main() {
    if !cfg!(target_os = "macos") {
        panic!("mlxr-sys currently only supports macOS on Apple Silicon");
    }
    ...
    // per-fingerprint `OUT_DIR`. `cargo clippy` fingerprints `mlxr-sys`
    ...
    //
    // OUT_DIR = <target>/<profile>/build/mlxr-sys-<hash>/out; three parents up is
    // <target>/<profile>.
