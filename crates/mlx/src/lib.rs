...
//! array framework, built on top of the [`mlxr-sys`] FFI layer.
//!
...
pub fn version() -> String {
    ...
    unsafe {
        let mut s = mlxr_sys::mlx_string_new();
        mlxr_sys::mlx_version(&mut s);
        let v = CStr::from_ptr(mlxr_sys::mlx_string_data(s))
            .to_string_lossy()
            .into_owned();
        mlxr_sys::mlx_string_free(s);
        v
    }
}
